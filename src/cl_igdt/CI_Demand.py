...
def read_historical_demand_data():
    ...
    filename = project_root / "data" / "historcial data_demand_69.xlsx"
...
def initialize_load_uncertainty_set(partition_num, iter, α_ini, nodes, T):
    P_load_Uset = {
        round(10 ** (-iter) * partition + α_ini, iter): np.zeros((nodes, T))
...
def extract_demand_samples(filename, sheet):
    system_demand = pd.read_excel(filename, sheet_name=sheet)

    D = {}
    D_k = {}

    for _, row in system_demand.iterrows():
...
def build_shortest_load_confidence_intervals(
    ...
    for t, _ in final_D_k.items():
        ...
        prob_densities_upper = np.insert(prob_densities_upper, 0, final_icdf_upper[t][
            0])  # the probability density of the first point == its cumulative probability
        ...
        for partition in range(partition_num):
            ...
            P_load_Uset[confidence_level][node_index, int(t[1:])] = -round(shortest_interval[1], 2)
...
def print_load_confidence_intervals(partition_num, iter, α_ini, node_index, confidence_interval):
    for partition in range(partition_num):
        confidence_level = round(10 ** (-iter) * partition + α_ini, iter)
        print(
...
def save_load_uncertainty_set(P_load_Uset, α_ini):
    np.savez(
        f"Bus69_P_load_Uset_{α_ini}.npz",
        **{str(key): value for key, value in P_load_Uset.items()}
    )


def CI_Demand(iter, partition_num, T, α_ini):
    filename, sheets = read_historical_demand_data()
    ...
    P_load_Uset = initialize_load_uncertainty_set(
        partition_num, iter, α_ini, nodes, T
    )
...
##----------------Construct the ambiguity set----------------
    for sheet in sheets:
        ...
        D, D_k, n_k, n = extract_demand_samples(filename, sheet)
...
##----------------calculate the shortest confidence interval----------------
        ...
        print_load_confidence_intervals(
            partition_num, iter, α_ini, index, confidence_interval
        )

    # save P_load_Uset for future direct loading
    save_load_uncertainty_set(P_load_Uset, α_ini)
