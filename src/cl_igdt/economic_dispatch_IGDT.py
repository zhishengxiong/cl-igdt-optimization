    ...
    for t in range(T):
        if t == 0:
            for i in range(len(ESS_node)):
                m.addConstr(ESS_E[i, t] == ESS_Eini[i] + ESS_eff[i] * ESS_pch[i, t] - ESS_pdis[i, t] / ESS_eff[i], name=f"ESS_energy_rule_{i}_{t}")
        else:
            for i in range(len(ESS_node)):
                m.addConstr(ESS_E[i, t] == ESS_E[i, t - 1] + ESS_eff[i] * ESS_pch[i, t] - ESS_pdis[i, t] / ESS_eff[i], name=f"ESS_energy_rule_{i}_{t}")

    for t in range(T):
        for i in range(len(ESS_node)):
            m.addConstr(ESS_pch[i, t] <= ESS_u[i, t] * ESS_pmax[i], name=f"ESS_ch_limit_{i}_{t}")
            m.addConstr(ESS_pdis[i, t] <= (1 - ESS_u[i, t]) * ESS_pmax[i], name=f"ESS_dis_limit_{i}_{t}")

    for n in range(num_nodes):
        if n + 1 in G_node:  # 发电机节点
            for t in range(T):
                i = G_node.index(n + 1)
                m.addConstr(P_net[n, t] == P_load[n, t] + G_p[i,t], name=f'P_net_rule_{n}_{t}')
        elif n + 1 in PV_node:  # 光伏节点
            for t in range(T):
                i = PV_node.index(n + 1)
                m.addConstr(P_net[n, t] == P_load[n, t] + PV[i,t], name=f'P_net_rule_{n}_{t}')
        elif n + 1 in ESS_node:  # 储能节点
            for t in range(T):
                i = ESS_node.index(n + 1)
                m.addConstr(P_net[n, t] == P_load[n, t] + ESS_pdis[i, t] - ESS_pch[i, t], name=f'P_net_rule_{n}_{t}')
        else:
            for t in range(T):  # 纯负荷节点
...
## ---------  Second-stage constraints --------------
    m.addConstr(G_p_reg == G_p + G_p_cor, name=f"G_reg_rule")
    m.addConstr(G_p_cor_abs >= G_p_cor, name=f"G_abs_pos_rule")
    m.addConstr(G_p_cor_abs >= -G_p_cor, name=f"G_abs_neg_rule")
    ...
    for n in range(num_nodes):
        if n + 1 in G_node:  # 发电机节点
            for t in range(T):
                i = G_node.index(n + 1)
                m.addConstr(P_net_2S[n, t] == P_load_real[n, t] + G_p_reg[i,t], name=f"P_net_2S_rule_{n}_{t}")
                m.addConstr(Q_net[n, t] == Q_load[n, t] + G_q[i,t], name=f"Q_net_rule_{n}_{t}")
        elif n + 1 in PV_node:  # 光伏节点
            for t in range(T):
                i = PV_node.index(n + 1)
                m.addConstr(P_net_2S[n, t] == P_load_real[n, t] + PV_real[i,t], name=f"P_net_2S_rule_{n}_{t}")
                m.addConstr(Q_net[n, t] == Q_load[n, t], name=f"Q_net_rule_{n}_{t}")
        elif n + 1 in ESS_node:  # 储能节点
            for t in range(T):
                i = ESS_node.index(n + 1)
                m.addConstr(P_net_2S[n, t] == P_load_real[n, t] + ESS_pdis[i, t] - ESS_pch[i, t], name=f'P_net_2S_rule_{n}_{t}')
                m.addConstr(Q_net[n, t] == Q_load[n, t], name=f"Q_net_rule_{n}_{t}")
        else:   # 纯负荷节点
            for t in range(T):
    ...
    for t in range(T):
        ...
        m.addConstr(v[:,t] == v0 + (R_prime@P_net_2S[:,t] + X_prime@Q_net[:,t]), name=f"v_rule_{t}")

##------  Uncertainty set and stuff--------------##
    ## Partition of objective function
    for i in range(partition_num):
        m.addConstr(theta_s[i] >= (α_ini + 10**(-iter) * i) * theta_u[i], name=f"theta_s_limt1_{i}")
        m.addConstr(theta_s[i] <= (α_ini + 10**(-iter) * (i+1)) * theta_u[i], name=f"theta_s_limt2_{i}")

    m.addConstr(quicksum(theta_u[i] for i in range(partition_num)) == 1, name=f"obj_u_rule")
    m.addConstr(quicksum(theta_s[i] for i in range(partition_num)) == theta, name=f"theta_rule")

    ## Partition of uncertainty set
    m.addConstr(P_load_real == quicksum(P_load_Uset[round(10 ** (-iter) * i + α_ini, iter)] * theta_u[i] for i in range(partition_num)), name=f"Load_Uset_rule")
    m.addConstr(Q_load == P_load_real * 0.9, name=f"Reactive_Load_rule")
    m.addConstr(PV_real == quicksum(PV_Uset[round(10 ** (-iter) * i + α_ini, iter)] * theta_u[i] for i in range(partition_num)), name=f"PV_Uset_rule")
    ...
    m.addConstr(total_cost <= 223458.23*1.3, name='Budget_limit')

    ## Auxiliary constraints
    for t in range(T):
        m.addConstr(flow_pos_auxi[t] - flow_neg_auxi[t] == P_flow_2S[0, t] - P_flow[0, t], name='Flow_diff_rule')
        m.addConstr(flow_pos_auxi[t] <= flow_u[t] * P_flow_up * 2, name='Flow_abs_pos_rule')
        m.addConstr(flow_neg_auxi[t] <= (1 - flow_u[t]) * P_flow_up * 2, name='Flow_abs_neg_rule')
